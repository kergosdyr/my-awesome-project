...
- `make test`는 coding 모듈 전체 과제를 실행한다. `make check-study`는 두 모듈 컴파일·완료 코딩28개·활성 커머스18개를 검사한다. 전체 과제는 챌린지 루트의 `./gradlew test --continue`로 실행하며 미완성 실패를 감추지 않는다. `npm ci`는 최초 설정 또는 의존성 변경 때 `make setup-blog`로 분리한다.
