...
with (out/'server.log').open('w') as log:
    try:
        server=subprocess.Popen([str(project/'gradlew'),'-p',str(project),'loadServer','--no-daemon','--console=plain'],stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
...
''')
        ...
        with (out/'k6.log').open('w') as klog:
            k6=subprocess.Popen(['k6','run',str(exercise/'tools/reservation-load/mixed-reservations.js')],cwd=exercise,env=env,stdout=klog,stderr=subprocess.STDOUT)
