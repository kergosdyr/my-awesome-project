...
현재 작업 코드는 [ReservationService.java](src/main/java/challenge/ReservationService.java)와
[CancellationService.java](src/main/java/challenge/CancellationService.java)다.
...
[ORDER.md](ORDER.md)는 선택적인 설명 메모다. 메시지로 설명해도 되며 파일 작성은 필수가 아니다.
...
```sh
cd /Users/justin/IdeaProjects/my-project/study/challenge-120
./gradlew :day-003:test -Pmysql --tests challenge.MixedReservationServiceTest
python3 exercises/day-003/load/run.py
```

IntelliJ 실행 구성은 `Day 003 - Backend MySQL`이다. k6 실행·FD 관찰·정리는 AI가 담당한다.
[k6 스크립트](load/mixed-reservations.js)는 60초 동안 50→200→500→1,000 VU로 올렸다가 낮춘다.
...
`python3 load/run.py`는 k6 내장 대시보드 데이터를 2초 간격으로 수집해
...
실시간 화면도 필요하면 `K6_WEB_DASHBOARD_PORT=5665 python3 load/run.py`로 실행하고
...
실행: `python3 load/run.py --nginx`. 로컬에 설치된 Nginx를 별도 프로세스·임의 loopback 포트로 실행하고 종료한다.
