...
상단 실행 구성에서 `Day 001 - Coding`, `Day 001 - Backend`, `Day 001 - All`을 선택하고 ▶를 누르세요. 테스트 파일의 클래스/메서드 옆 실행 버튼도 사용할 수 있습니다. Backend 기본 실행은 H2이므로 Docker가 필요 없습니다. Day 폴더가 추가되면 Gradle 새로고침으로 불러옵니다.

터미널에서는 아래 명령도 그대로 사용할 수 있습니다. 기존 앱 소스는 변경하지 않습니다.

## 먼저 Coding 25분

수정할 파일: [LongestBatch.java](src/main/java/challenge/LongestBatch.java)

```bash
bash study/challenge-120/exercises/day-001/run.sh coding
```
...
수정할 파일: [ReservationService.java](src/main/java/challenge/ReservationService.java), 필요하면 [schema.sql](src/main/resources/schema.sql).

```bash
bash study/challenge-120/exercises/day-001/run.sh backend
```
...
CHALLENGE_JDBC_URL='jdbc:mysql://127.0.0.1:13316/challenge_day001?useSSL=false&allowPublicKeyRetrieval=true&connectTimeout=5000&socketTimeout=15000' CHALLENGE_DB_USER=challenge CHALLENGE_DB_PASSWORD=challenge bash run.sh backend --rerun-tasks
...
미구현 상태의 TODO 예외와 테스트 실패는 의도된 시작 상태입니다. 컴파일 실패·DB 접속 오류와 구분하세요. 전체 테스트는 `bash run.sh all`, 컴파일만 확인하려면 `bash run.sh compile`입니다(이 폴더 기준).
