...
public class ApiErrors {
    ...
    ResponseEntity<ErrorResponse> invalid() {
        return ResponseEntity.badRequest()
                .body(new ErrorResponse("INVALID_REQUEST", "옵션과 수량(1~5개)을 확인해 주세요."));
    }

    @ExceptionHandler(UnsupportedOperationException.class)
    ResponseEntity<ErrorResponse> unfinished() {
        return ResponseEntity.status(501)
                .body(new ErrorResponse("NOT_IMPLEMENTED", "요청한 결제 처리가 아직 구현되지 않았습니다."));
    }
    ...
    ResponseEntity<ErrorResponse> invariant() {
        return ResponseEntity.status(409)
                .body(
                        new ErrorResponse(
                                "PAYMENT_CONFLICT", "결제 기록을 갱신하지 못했습니다. 기존 주문 상태를 확인해 주세요."));
    }
}
