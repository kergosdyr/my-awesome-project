...
abstract class CommerceHttpSupport {
    ...
    static void database(DynamicPropertyRegistry properties) {
        properties.add(
                ...
                () -> MYSQL ? DB.getJdbcUrl() : "jdbc:h2:mem:commerce-test;DB_CLOSE_DELAY=-1");
    ...
    @Autowired challenge.commerce.domain.payment.PaymentRepository paymentStore;
    @Autowired OrderJpaRepository orders;
    @Autowired ProductOptionJpaRepository options;
    ...
    long order() throws Exception {
        var response = request("POST", "/api/orders", "{\"optionId\":101,\"quantity\":1}");
