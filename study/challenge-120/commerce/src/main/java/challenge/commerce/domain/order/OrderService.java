 ...
 * 관찰: ./gradlew :commerce:priceExperiment / 완료 검사: ./gradlew :commerce:test --tests '*PriceConsentTest'
...
public class OrderService {
    ...
    public OrderEntity create(CreateOrderCommand command) {
        ...
        var productResult = products.readForOption(command.optionId());
        productResult.product().validatePrice(command.displayedUnitPrice());
        stock.allocate(command.optionId(), command.quantity());
        return orders.create(productResult, command.optionId(), command.quantity());
    }
}
